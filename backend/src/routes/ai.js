 ...
 */
async function buildLiveDataContext(groupId) {
  const period = new Date().toISOString().slice(0, 7); // YYYY-MM
  try {
    const [summary, contributions, transactions, members] = await Promise.all([
    ...
    const nameById = {};
    for (const m of members) nameById[m.user_id] = m.name;

    const contribLines = contributions.map((c) => {
      const name = nameById[c.user_id] ?? c.user_id;
      const statusLabel = c.status === "paid" ? "lunas" : c.status === "partial" ? "sebagian" : "belum bayar";
      return `  - ${name}: ${statusLabel} (${fmt(c.paid_amount)} dari ${fmt(c.expected_amount)})`;
    });

    const contribBlock = contribLines.length > 0
      ? [`### Status Iuran Anggota (${period})`, ...contribLines]
      : [`### Status Iuran Anggota (${period})`, "  Tidak ada data iuran untuk periode ini."];
    ...
    const notPaid = contributions
      ...
      .map((c) => {
        const name = nameById[c.user_id] ?? c.user_id;
        const label = c.status === "partial" ? "sebagian" : "belum bayar";
        return `  - ${name} (${label})`;
      });

    const unpaidBlock = notPaid.length > 0
      ? ["### Anggota Belum/Belum Lunas Bayar", ...notPaid]
      : [`### Anggota Belum/Belum Lunas Bayar`, `  Semua anggota sudah lunas untuk ${period}.`];

    // ── Recent transactions ───────────────────────────────────────────────
    const txLines = transactions.slice(0, 15).map((t) => {
      const sign = t.type === "income" ? "+" : "-";
      return `  - [${t.date}] ${sign}${fmt(t.amount)} | ${t.description} (${t.category})`;
    });

    const txBlock = txLines.length > 0
...
router.post("/ai/chat", aiRateLimiter, async (req, res) => {
  ...
  if (!parseResult.success) {
    const firstIssue =
      (parseResult.error.errors[0] && parseResult.error.errors[0].message) ||
      "Invalid request";
  ...
  const liveContext = isAvailable()
    ? await buildLiveDataContext(demoGroupId)
    : "";
  ...
  try {
    ...
    const result = await callLangflow(enrichedMessage, sessionId);
    ...
    if (err instanceof LangflowTimeoutError) {
      ...
      return res
        .status(504)
        .json({ error: "The AI service took too long to respond. Please try again.", requestId });
    }
    if (err instanceof LangflowUpstreamError) {
      console.warn("[AI Chat] upstream error", { requestId, upstreamStatus: err.upstreamStatus });
      return res
        .status(502)
        .json({ error: "The AI service is temporarily unavailable. Please try again.", requestId });
    }
    console.error("[AI Chat] unexpected error", { requestId, name: err && err.name });
    return res
      .status(500)
      .json({ error: "An unexpected error occurred. Please try again.", requestId });
  }
});
